...
class SaasRegistryHelper {
    ...
    static async sendResult(callbackUrl, tenantId, { status, message, subscriptionUrl, saasCallbackUrl, saasRequestPayload }, authHeader) {
        ...
        if (saasCallbackUrl) {
            ...
            const {saas_registry_url} = SaasRegistryHelper._getSaasCredentials();
    ...
    static async _getAuthToken() {
        ...
        const credentials = this._getSaasCredentials();
        ...
        try {
            const saasRegistryTokenResponse = await JwtHelper.getSaasRegistryToken(url, clientid, clientsecret);
    ...
    static _getSaasCredentials() {
        const credentials = typeof cds.env.get('requires.multitenancy.credentials') === 'object'
            ? cds.env.get('requires.multitenancy.credentials')
            : JSON.parse(process.env.VCAP_SERVICES)['saas-registry'][0].credentials;
        return credentials;
    }
}
