...
class HttpHelper {
    ...
    static checkMetadataScopeAndRespond(context) {
        if (cds.env.get('mtx.security.metadata-scope-checks')) {
            HttpHelper.checkAnyScopeAndRespond(context, 'DEPLOYMENT');
        } else {
            this.logger.warn(`Metadata APIs are not scope-checked by default. It is recommended to enable them by setting 'cds.env.mtx.security.metadata-scope-checks' to 'true'`);
        }
    }
    ...
    static checkAnyScopeAndRespond(context, scopes) {
        ...
        scopes = scopes.reduce((res, scope) =>
            res.concat(SecurityHelper.getConfiguredScopes(scope) || scope)
        , []);
